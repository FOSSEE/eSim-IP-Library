...
void cm_jkff32(ARGS) 
{
    ...
    if(INIT)
    {
		...
		int Ii;
		for(Ii=0;Ii<PORT_SIZE(j);Ii++)
		{
			LOAD(j[Ii])=PARAM(input_load); 
		}
		for(Ii=0;Ii<PORT_SIZE(k);Ii++)
		{
			LOAD(k[Ii])=PARAM(input_load); 
		}
